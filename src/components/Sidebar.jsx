...
import {
    ...
    MdHandshake, MdLocalShipping, MdSettings, MdCreditCard, MdDownload, MdLogout
} from "react-icons/md";
...
function Sidebar({ userData, onLogout }) {
    ...
    useEffect(() => {
        ...
        const fetchLowProducts = async () => {
            try {
                ...
                } else if (items && items.products && Array.isArray(items.products)) {
                    setLowProducts(items.products.filter(p => p.status === 'ناقص').length);
                } else {
                    setLowProducts(0);
                }
            } catch (err) {
    ...
    return (
        <>
            ...
            {/* الـ Modal الخاص بتأكيد تسجيل الخروج */}
            {showLogOutModal && (
                <LogOutModal
